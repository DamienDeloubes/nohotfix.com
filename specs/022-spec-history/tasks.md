...
- [X] T003 [P] Add `SpecChangelogEntry` interface to `packages/domains/audit/src/types.ts` (domain-internal read model with `isRemovedMember: boolean`). Import `SpecHistoryAction` from `@releasepilot/shared` — do NOT redefine it locally
...
- [X] T014 [P] [US1] Create `useSpecHistory` hook in `packages/domains/authoring/src/ui/hooks/use-spec-history.ts`. Accept `orgSlug`, `specId`, and `queryKey` as parameters (query key centralisation policy). Use `useApiQuery` from `@releasepilot/api-client` to fetch from `/api/orgs/${orgSlug}/specs/${specId}/history`. Validate response with `SpecHistoryResponseSchema`
