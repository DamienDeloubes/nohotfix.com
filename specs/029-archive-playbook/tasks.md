...
- [ ] T016 Add run creation guard: in the `POST /api/runs` route handler in `apps/api/src/routes/execution.ts`, check if the source playbook is archived before starting a run. If archived, throw `PlaybookArchivedError` (import from `@nohotfix/domain-authoring`) — **DEFERRED: route is a 501 stub**
