...
| # | Principle | Check |
|---|-----------|-------|
| I | **Bounded Context Integrity** — Primary context: **Authoring** (spec lifecycle). Cross-context dependency: **Audit** (changelog recording via API-layer orchestration, not direct import). Domain packages import only `@releasepilot/shared`. Consumer rules: `apps/api` imports domain logic only; `apps/app` imports both logic and UI. | PASS |
