...
- [x] T021 [P] [US4] Create `SpecTag` value object in `packages/domains/authoring/src/entities/value-objects/spec-tag.ts`: private constructor, static `create(raw: string)` applies `toKebabCase()` from `@nohotfix/shared`, validates 1–30 chars after transform, throws `AuthorSpecTagsInvalidError` if empty or too long. Add `equals()` and `toString()`. (pattern: /project:new-entity)
