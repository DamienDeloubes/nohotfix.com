...
| # | Principle | Check |
|---|-----------|-------|
| I | **Bounded Context Integrity** — Primary context: **Authoring** (`packages/domains/authoring/`). No cross-domain imports. Domain package depends only on `@releasepilot/shared` + `zod`. UI hooks in `packages/domains/authoring/src/ui/`. Route files in `apps/app` compose domain UI. API routes in `apps/api/src/routes/authoring.ts`. | ✅ |
