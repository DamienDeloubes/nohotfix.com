...
- **[Billing & Subscription Management](features/must-have/billing-subscription.md)** — Stripe-backed subscription management: 14-day trial (clock starts at org creation), 3-day grace period, full-page access block on expiry, Admin-only upgrade flow via Stripe Checkout, billing management via Stripe Customer Portal.
...
- **[Artifact-gated spec execution](features/must-have/artifact-gated-spec-execution.md)** — A spec cannot be marked passed until all declared evidence requirements (file upload, table, measured value, or URL) are satisfied. This enforcement cannot be bypassed.
...
- **[Email Notifications](features/must-have/email-notifications.md)** — Four triggers: invitation sent, run ready for go/no-go, go/no-go decision recorded, run abandoned. All team members notified of decisions and abandonment; Admins notified of go/no-go readiness.
...
| Viewer role                                                                               | No validated use case in v1; add when a customer asks for it                                                                                                                                                                                                          |
...
| Jira / Linear integration                                                                 | Complexity; webhooks/API first                                                                                                                                                                                                                                        |
...
- **Goal**: Revenue from day one. No permanent free tier.
- **Model**: Flat monthly fee per team — one price regardless of seat count. Not per-seat, not usage-based.
- **Trial**: 14-day free trial on signup. Full product access during the trial. After 14 days, the team must upgrade to a paid plan or lose access. No exceptions, no extensions by default.
- **Tiers / price points**: TBD — pending market research and competitive benchmarking. Must be defined before the first external customer conversation. Consider anchoring to the cost of one compliance incident or one failed audit to frame the value.
...
- **Notifications**: Email only in v1. No in-app notification center. No Slack/Teams integration.
