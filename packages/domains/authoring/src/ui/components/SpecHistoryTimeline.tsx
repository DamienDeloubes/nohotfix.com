...
interface SpecHistoryTimelineProps {
  entries: SpecHistoryEntry[];
}

export function SpecHistoryTimeline({ entries }: SpecHistoryTimelineProps): React.ReactElement {
  if (entries.length === 0) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
      {entries.map((entry) => (
        ...
        >
          ...
          <span style={{ color: '#374151' }}>{describeHistoryAction(entry)}</span>
